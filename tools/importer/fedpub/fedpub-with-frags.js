...
const addFragmentLinktoPage = (main, fragmentName, document) => {
    const link = document.createElement('a');
    link.setAttribute('href', `fragments/${fragmentName.toLowerCase().replace(/\s+/g, '-')}`);
    link.innerHTML = 'Promotion Fragment';
    main.append(link);
};
...
export default {
  ...
  transform: ({
    ...
    const fragmentQuery = document.querySelector('.promotion');
    const fragment = fragmentQuery.cloneNode(true);
    const fragmentName = fragment.getAttribute('data-promotion');
    ...
    if (fragment) {
        addFragmentLinktoPage(main, fragmentName, document);
    }
    ...
    return [{
      ...
      {
        element: fragment,
        path: `fragments/${fragmentName.toLowerCase().replace(/\s+/g, '-')}`,
    },
    ];
